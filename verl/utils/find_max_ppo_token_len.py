# -*- coding: utf-8 -*-
"""[VERL] ppo_max_token_len_per_gpu limit finder.

Extracted from ``DataParallelPPOActor`` to keep ``dp_actor.py`` lean. Enabled by the env var
``VERL_FIND_MAX_PPO_TOKEN_LEN``; ``update_policy`` calls
...
def find_max_ppo_token_len_per_gpu(actor, data):
    ...
    Mechanism (enabled by env ``VERL_FIND_MAX_PPO_TOKEN_LEN``):
    ...
    * Every probe is identical across ranks (``L`` derives from config, not data) and the OK/peak
    ...
    Tunable via env vars (all optional):
      ``VERL_FIND_MAX_START``      initial probe size (default: current value)
      ``VERL_FIND_MAX_CAP``        hard upper bound on probe size (default: 16x current)
      ``VERL_FIND_MAX_BUDGET_S``   wall-clock budget in seconds (default: 600)
      ``VERL_FIND_MAX_SEQ``        cap on a single synthetic sequence (default: current value)
      ``VERL_FIND_MAX_SAFE_FRAC``  stop ramping at this mem fraction (default: 0.80)
      ``VERL_FIND_MAX_CEIL_FRAC``  extrapolated OOM ceiling fraction (default: 0.92)
      ``VERL_FIND_MAX_MARGIN``     head-room applied to the result (default: 0.90)
    ...
    start = max(512, _envint("VERL_FIND_MAX_START", max(1024, cur)))
    hard_cap = _envint("VERL_FIND_MAX_CAP", cur * 16)
    budget_s = _envint("VERL_FIND_MAX_BUDGET_S", 600)
    max_seq_cap = max(8, _envint("VERL_FIND_MAX_SEQ", cur))
    ...
    safe_frac = float(os.environ.get("VERL_FIND_MAX_SAFE_FRAC", "0.80"))   # stop ramping here
    ceil_frac = float(os.environ.get("VERL_FIND_MAX_CEIL_FRAC", "0.92"))   # OOM ceiling estimate
    margin = float(os.environ.get("VERL_FIND_MAX_MARGIN", "0.90"))         # head-room on result
    ...
    raise RuntimeError(
        ...
        "WITHOUT VERL_FIND_MAX_PPO_TOKEN_LEN.\n"
        "======================================================================="
    )
